...
import threading
import pprint
from google.cloud import modelarmor_v1 # New Import
...
MODEL_ARMOR_TEMPLATE_ID=os.environ["MODEL_ARMOR_TEMPLATE_ID"]
...
# REQUIRED: Point the client to the regional REP (Regional Endpoint)
# Check if you region is set to us-central1 in the Model Armor config, if not change here as well:
...
try:
    # This checks if the template is even visible to your current credentials
    template = ma_client.get_template(name=MODEL_ARMOR_TEMPLATE_ID)
...
def sanitize_with_model_armor(text, user_id):
    """
    Sends text to Model Armor for inspection based on the security template.
    """
    ...
    try:
        ...
        # Check if the content was blocked/filtered
        ...
        if int(response.sanitization_result.filter_match_state) == 2:
            # You can customize this message or return the 'invocation_result' from MA
            return None, "Policy Violation: The request or response contains restricted content."
        
        # Return the potentially sanitized text
            #return response.sanitization_result.sanitized_user_prompt_data.text, None
        return text, None

    except Exception as e:
        print(f"Model Armor Error: {e}")
        return text, None # Fail open or closed based on your preference
...
def chat():
    ...
    if error:
        return jsonify({"reply": error, "narrative": [{"agent": "Security", "action": "Blocking restricted input", "type": "system_event"}]})

    content = genai_types.Content(
        role='user',  # Standard role identifier for user input
        parts=[genai_types.Part(text=sanitized_input)]  # The actual text content
    )
    ...
    async def run_and_collect():
        ...
        try:
            ...
            # --- STEP 2: SHIELD OUTPUT ---
            # Inspect the final answer from the orchestrator before the user sees it
